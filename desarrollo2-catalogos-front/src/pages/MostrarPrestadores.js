...
export default function MostrarPrestadores() {
  ...
  const [reviewsOwner, setReviewsOwner] = useState(null);
  ...
  const fullNameFromUser = (u) => {
    ...
    return n || u?.nombreCompleto || u?.name || u?.username || null;
  };
  ...
  const fetchRows = useCallback(async () => {
    try {
      ...
      const mapped = onlyActive.map((p) => {
        ...
        const calle = p?.calle ?? p?.direccion_calle ?? p?.direccion?.calle ?? null;
        const numero = p?.numero ?? p?.direccion_numero ?? p?.direccion?.numero ?? null;

        let direccion = "-";
        if (calle) {
          direccion = numero ? `${calle} ${numero}` : `${calle}`;
        } else {
          direccion = p?.direccion || p?.domicilio || p?.address || "-";
        }
  ...
  const handleSaveEdit = async (formValues) => {
    ...
    try {
      ...
      const nuevasZonasIds = nuevasZonasIdsStrings.map((z) => parseInt(z, 10));
      ...
      const hasFormChanges = comparables.some((k) => {
        const nuevo = (datosPrestador?.[k] ?? "").toString().trim();
        const viejo = (selected?.[k] ?? "").toString().trim();
        return nuevo !== viejo;
      });

      if (hasFormChanges) {
        const updatePayload = {
          nombre: (datosPrestador.nombre ?? "").trim(),
          apellido: (datosPrestador.apellido ?? "").trim(),
          email: (datosPrestador.email ?? "").trim(),
          telefono: (datosPrestador.telefono ?? "").trim(),
          estado: (datosPrestador.estado ?? "").trim(),
          ciudad: (datosPrestador.ciudad ?? "").trim(),
          calle: (datosPrestador.calle ?? "").trim(),
          numero: (datosPrestador.numero ?? "").toString().trim(),
          // opcionales
          piso: (datosPrestador.piso ?? "").toString().trim(),
          departamento: (datosPrestador.departamento ?? "").toString().trim(),
        };
      ...
      const hasZonaChanges =
        JSON.stringify([...nuevasZonasIds].sort()) !==
        JSON.stringify([...originalZonasIds].sort());
        ...
        {reviewsLoading ? (
          ...
          <MC.Stack gap="md">
            {currentReviews.map((rev, idx) => {
              ...
              const estrellas = Number(rev?.estrellas ?? rev?.puntuacion ?? rev?.rating ?? 0);
              const userId = extractUserId(rev);
              const autorNameFromObj = rev?.usuario?.nombre || rev?.autor || null;
              const autor =
              ...
              return (
                <MC.Paper key={rev?.id ?? idx} withBorder radius="md" p="md">
                  <MC.Group justify="space-between" align="center" mb="xs">
                    <MC.Rating readOnly value={isNaN(estrellas) ? 0 : estrellas} fractions={10} color="yellow" size="md" />
                    {fecha && (
