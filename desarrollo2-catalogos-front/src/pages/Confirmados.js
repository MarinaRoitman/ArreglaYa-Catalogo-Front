...
export default function Confirmados({ data, rechazar }) {
  ...
  // --- Lógica del modal de confirmación (COMENTADA) ---
  /*
    ...
    <AppLayout>
      ...
      >
        ...
        {filteredData.length === 0 ? (
          ...
          // Antes: <CardsMobile rows={filteredData} type="confirmados" rechazar={askDelete} />
          <CardsMobile rows={filteredData} type="confirmados" />
        ) : (
          // Antes: <TableComponent rows={filteredData} type="confirmados" rechazar={askDelete} />
          <TableComponent rows={filteredData} type="confirmados" />
        )}
      </Paper>

      {/* Modal eliminado temporalmente */}
      {/*
      <ConfirmDelete
        opened={confirmOpen}
