...
<MC.Box>
    ...
    >
        ...
        highlightOnHover
        withColumnBorders
        style={{ minWidth: 1080, tableLayout: "fixed" }}
        >
        <MC.Table.Thead>
            <MC.Table.Tr>
            ...
            <MC.Table.Th>
                ...
                onChange={(e) => handleFilterChange("nombre", e.currentTarget.value)}
                size="xs"
                radius="sm"
                />
            </MC.Table.Th>
            <MC.Table.Th>
                ...
                onChange={(e) => handleFilterChange("telefono", e.currentTarget.value)}
                size="xs"
                radius="sm"
                />
            </MC.Table.Th>
            <MC.Table.Th>
                ...
                onChange={(e) => handleFilterChange("direccion", e.currentTarget.value)}
                size="xs"
                radius="sm"
                />
            </MC.Table.Th>
            <MC.Table.Th>
                ...
                placeholder="Buscar..."
                value={filters.zonas}
                onChange={(e) => handleFilterChange("zonas", e.currentTarget.value)}
                size="xs"
                radius="sm"
                />
            </MC.Table.Th>
            <MC.Table.Th>
                ...
                placeholder="Buscar..."
                value={filters.dni}
                onChange={(e) => handleFilterChange("dni", e.currentTarget.value)}
                size="xs"
                radius="sm"
                />
            </MC.Table.Th>
            ...
            <MC.Table.Th /> {/* columna acciones sin filtro */}
            </MC.Table.Tr>
        </MC.Table.Thead>
        ...
        <MC.Table.Tbody>
            ...
            <MC.Table.Tr key={r.id ?? `${r.email}-${r.dni}`}>
                ...
                <MC.Table.Td style={{ width: ACTIONS_COL_WIDTH, textAlign: "center" }}>
    ...
    {/* MODO MOBILE: se mantiene igual */}
    <MC.Stack gap="sm" hiddenFrom="sm">
    {filtered.map((r) => (
        <MC.Paper key={r.id} p="md" radius="lg" withBorder style={{ background: "white" }}>
        ...
        <MC.Text size="sm">
            <strong>Dir:</strong> {r.direccion || "-"}
        </MC.Text>
        <MC.Text size="sm">
            <strong>Zonas:</strong> {Array.isArray(r.zonas) ? r.zonas.join(", ") : r.zonas}
        </MC.Text>

        <MC.Group gap="xs" mt="sm">
            <MC.ActionIcon variant="light" color="blue" onClick={() => onEdit?.(r)}>
            <IconPencil size={18} />
            </MC.ActionIcon>
            <MC.ActionIcon variant="light" color="yellow" onClick={() => onRate?.(r)}>
            <IconStar size={18} />
            </MC.ActionIcon>
            <MC.ActionIcon variant="light" color="red" onClick={() => onDelete?.(r)}>
            <IconX size={18} />
            </MC.ActionIcon>
        </MC.Group>
        </MC.Paper>
