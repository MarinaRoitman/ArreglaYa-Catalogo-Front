...
useEffect(() => {
setErr("");
const initial = isEdit ? (zona?.nombre ?? "") : "";
setNombre(initial);
...
// 👇 NUEVO: solo habilitar si hubo cambios
const isChanged = nombreTrim !== originalNombre.trim();

const canSave = nombreTrim.length > 0 && !isDuplicate && !saving;

const handleSubmit = async () => {
if (!canSave || (isEdit && !isChanged)) return;
...
>
    ...
    <Group justify="flex-end" mt="xs">
        ...
        loading={saving}
        disabled={!canSave || (isEdit && !isChanged)}   
        >
