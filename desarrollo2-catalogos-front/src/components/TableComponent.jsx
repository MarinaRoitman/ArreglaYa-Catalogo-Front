...
export default function TableComponent({ rows = [], aprobar, rechazar, calificar, type = 'solicitudes' }) {
  ...
  const handleConfirmSubmit = ({ id, fecha, montoTotal }) => {
    if (typeof aprobar === "function") {
      // Le pasamos los datos a la función 'aprobar' que viene desde App.js
              ...
              <MC.Table.Tr key={row.id}>
                ...
                <MC.Table.Td>
                  <MC.Group gap="xs" justify="center">
                    ...
                    {/* --- ¡CORRECCIÓN CLAVE AQUÍ! Se cambió 'job.estado' por 'row.estado' --- */}
                    {type === 'confirmados' && (
                      row.estado === 'aprobado_por_usuario' ? (
                        <MC.ActionIcon variant="light" color="green" aria-label="Aprobado por Cliente">
                          <IconCircleCheck size={18} />
                        </MC.ActionIcon>
                      ) : (
                        <MC.ActionIcon variant="light" color="blue" aria-label="Esperando al Cliente">
